...
extension ProjectManagerViewController: UITableViewDropDelegate {
    ...
    */
    ...
    func tableView(_ tableView: UITableView, performDropWith coordinator: UITableViewDropCoordinator) {
        ...
        let currentTableView = checkedTableView(currentTableView: tableView)
        
        if let indexPath = coordinator.destinationIndexPath {
            destinationIndexPath = indexPath
            print(destinationIndexPath)
        } else {
            let section = currentTableView.numberOfSections
            let row = 0
        ...
        coordinator.session.loadObjects(ofClass: CellData.self) { items in
            ...
            for (index, item) in stringItems.enumerated() {
                let indexPath = IndexPath(row: destinationIndexPath.row, section: destinationIndexPath.section + index)
                self.addItem(currentTableView: currentTableView, item, at: indexPath.section)
                indexPaths.append(indexPath)
            }

            currentTableView.insertRows(at: indexPaths, with: .automatic)
        }
    }
    
    func checkedTableView(currentTableView: UITableView) -> UITableView {
